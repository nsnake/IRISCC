...
require_once ('agent_queue_logs.grid.inc.php');
require_once ('include/xajaxGrid.inc.php');
require_once ('include/astercrm.class.php');
require_once ('include/common.class.php');
require_once ("agent_queue_logs.common.php");
...
function createGrid($start = 0, $limit = 1, $filter = null, $content = null, $order = null, $divName = "grid", $ordering = "",$stype=array()){
	...
	$headers[] = $locate->Translate("Queue");
	$headers[] = $locate->Translate("Account");
	$headers[] = $locate->Translate("Pausetime");
	$headers[] = $locate->Translate("Reasion");
	...
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","queue","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","account","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","pausetime","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","reasion","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","cretime","'.$divName.'","ORDERING");return false;\'';

	// Select Box: fields table.
	$fieldsFromSearch = array();
	$fieldsFromSearch[] = 'action';
	$fieldsFromSearch[] = 'queue';
	$fieldsFromSearch[] = 'account';
	$fieldsFromSearch[] = 'pausetime';
	$fieldsFromSearch[] = 'reasion';
	$fieldsFromSearch[] = 'cretime';

	// Selecct Box: Labels showed on search select box.
	$fieldsFromSearchShowAs = array();
	$fieldsFromSearchShowAs[] = $locate->Translate("action");
	$fieldsFromSearchShowAs[] = $locate->Translate("queue");
	$fieldsFromSearchShowAs[] = $locate->Translate("account");
	$fieldsFromSearchShowAs[] = $locate->Translate("pausetime");
	$fieldsFromSearchShowAs[] = $locate->Translate("reasion");
	...
	$table->addRowSearchMore("agent_queue_logs",$fieldsFromSearch,$fieldsFromSearchShowAs,$filter,$content,$start,$limit,0,$deleteBtnFlag,$typeFromSearch,$typeFromSearchShowAs,$stype);
	...
	// Change here by the name of fields of its database table
		...
		$rowc[] = $row['reasion'];
		$rowc[] = $row['cretime'];
		$table->addRow("agent_queue_logs",$rowc,$editFlag,$deleteFlag,0,$divName,$fields);
 	}
...
function searchFormSubmit($searchFormValue,$numRows = null,$limit = null,$id = null,$type = null){
	...
	if($optionFlag == "export"  || $optionFlag == "exportcsv"){
		$joinstr = Customer::createSqlWithStype($searchField,$searchContent,$searchType,'ticket_op_log'); //得到要导出的sql语句
		$joinstr=ltrim($joinstr,'AND');
		$sql = "SELECT * from agent_queue_log ";
		if($joinstr != '') $sql .= " WHERE ".$joinstr;
		$_SESSION['export_sql'] = $sql.'';
		
		$objResponse->addAssign("hidSql", "value", $sql); //赋值隐含域
		$objResponse->addAssign("maintable", "value", 'agent_queue_log'); //赋值隐含域
		$objResponse->addAssign("exporttype", "value", $optionFlag);
		$objResponse->addScript("document.getElementById('exportForm').submit();");
	}elseif($optionFlag == "delete"){
		astercrm::deletefromsearch($searchContent,$searchField,$searchType,'agent_queue_log');
		$html = createGrid($searchFormValue['numRows'], $searchFormValue['limit'],'','',$order,$divName,$ordering,'');
		$objResponse->addClear("msgZone", "innerHTML");
		$objResponse->addAssign($divName, "innerHTML", $html);
	}elseif($type == "delete"){
		$res = Customer::deleteRecord($id,'agent_queue_log');
		if ($res){
...
function deleteByButton($f,$searchFormValue){
	...
	if(is_array($f['ckb'])){
		foreach($f['ckb'] as $vaule){
			$res_customer = astercrm::deleteRecord($vaule,'agent_queue_log');
		}
	}
	$searchContent = $searchFormValue['searchContent'];  //搜索内容 数组
	$searchField = $searchFormValue['searchField'];      //搜索条件 数组
...
function delete($ticketid = null){
	...
	$res = Customer::deleteRecord($ticketid,'agent_queue_log');
	if ($res){
