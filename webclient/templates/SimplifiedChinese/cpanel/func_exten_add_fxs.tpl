...
<form name="do_exten_add_fxs" method="POST" action="?action=do_exten_add_fxs" target="takefire">
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<tr>
			...
			<td height="30">
			<select size="1" name="devicestring">
		<%foreach from=$freechan item=eachone key=keyname %>
				<option value="<% $eachone %>">FXS / <% $eachone %></option>
		<%/foreach%>
			</select>
			</td>
		</tr>
