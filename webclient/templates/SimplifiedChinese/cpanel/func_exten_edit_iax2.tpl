...
<script language="javascript">
	...
	<h4>编辑IAX2分机 <% $extension.accountcode %> 信息</h4>

<form name="do_exten_edit_iax2" method="POST" action="?action=do_exten_edit_iax2&accountcode=<% $extension.accountcode %>" target="takefire">
			...
			<td colspan="3">
				<table border="0" cellspacing="0" id='advance' style='display:none'>
					...
					<tr>
						...
						<td height="30"><input type="text" id="iptext1" name="port" size="6" value="<% $extension.port %>"></td>
					</tr>
					<tr>
					...
					<tr>
						<td height="30" align="left" style="padding-left: 10px;padding-right: 40px">转发参数</td>
						<td height="30"><input type="radio" value="yes" <% if $extension.transfer eq 'yes' %>checked<%/if%> name="transfer">Yes <input type="radio" value="no"  <% if $extension.transfer eq 'no' %>checked<%/if%>  name="transfer">No <input type="radio" value="no"  <% if $extension.transfer eq 'mediaonly' %>checked<%/if%> name="transfer">Mediaonly</td>
					</tr>
					<tr>
