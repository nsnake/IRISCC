...
<div id="body-body">
	...
	<table border="0" width="95%" align="left" style="margin: 15px">
		...
		<tr>
			...
			<table border="0" align="left" style="margin-top: 20px">
				<tr>
					<td height="30" align="left"><input type="button" value="上载新的音乐" name="button" id='btn1' class='showpopDialog' func="pbx_musicmanager.php?action=func_musicmanager_add"></td>
				</tr>
			</table>
			</td>
		</tr>
		<tr>
			...
			<tr>
				<td height="30" align="left" width="50%">有效音乐文件&nbsp;(&nbsp;<% $from_cols %>&nbsp;-&nbsp;<% $to_cols %>&nbsp;)&nbsp;共&nbsp;<% $maxcount %>&nbsp;首</td>
			...
			<table width="100%" border="0" cellspacing="0" style="margin-top: 0px">
				<tr>
					<td height="30" align="center" style="border-bottom: #ACA8A1 1px solid;">音乐文件</td>
		...
		<%foreach from=$soundmusic_array item=eachone key=keyname %>
				<tr>
					<td height="30" align="left" style="border-bottom: #ACA8A1 1px solid;" bgcolor="<% if $keyname is even %>#FFFFFF<%else%>#F3F3F3<%/if%>">&nbsp;<b><% $eachone.filename %></td>
					<td height="30" align="left" style="border-bottom: #ACA8A1 1px solid;" bgcolor="<% if $keyname is even %>#FFFFFF<%else%>#F3F3F3<%/if%>">&nbsp;<% $eachone.extname %></td>
					...
					<td height="30" align="center" style="border-bottom: #ACA8A1 1px solid;" bgcolor="<% if $keyname is even %>#FFFFFF<%else%>#F3F3F3<%/if%>">&nbsp;<input type="button" value="试听 / 更新" name="button" id='btn1' class='showpopDialog' func="pbx_musicmanager.php?action=func_musicmanager_edit&id=<% $eachone.id %>">&nbsp;<% if $eachone.readonly eq '0' %><input type="button" value="删除" name="button" class="deletebtn" id='btn2' gotourl="pbx_musicmanager.php?action=do_musicmanager_delete&id=<% $eachone.id %>">&nbsp;<%/if%></td>
				</tr>
		<%/foreach%>
