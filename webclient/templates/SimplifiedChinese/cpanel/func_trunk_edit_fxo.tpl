...
$().ready(function() {
	...
	//提交时全选
	$("#do_trunk_edit_fxo").submit(function(){
	...
	<h4>编辑 PSTN FXO 模拟外线</h4>

<form id='do_trunk_edit_fxo' name="do_trunk_edit_fxo" method="POST" action="?action=do_trunk_edit_fxo&id=<% $trunk.id %>" target="takefire">
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<%foreach from=$trunk.channel_array item=chan %><option value="<% $chan %>"> FXO / <% $chan %> </option><%/foreach%>
						</select>
					</td>
				</tr>
			</table>
