...
$().ready(function() {
	...
	//提交时全选
	$("#do_trunk_edit_isdnpri").submit(function(){
	...
	<h4>编辑 E1 ISDNPRI 30B+D 数字中继</h4>

<form id='do_trunk_edit_isdnpri' name="do_trunk_edit_isdnpri" method="POST" action="?action=do_trunk_edit_isdnpri&id=<% $trunk.id %>" target="takefire">
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<%foreach from=$trunk.channel_array item=chan %><option value="<% $chan %>"> PRI / <% $chan %> </option><%/foreach%>
						</select>
					</td>
				</tr>
			</table>
