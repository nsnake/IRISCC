...
<form name="do_router_add" method="POST" action="?action=do_router_edit&id=<% $rule.id %>" target="takefire">
	...
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<%foreach from=$provider_array item=eachone key=keyname %>
				<option value="<% $eachone.id %>" <% if $eachone.id eq $rule.match_callergroup %>selected<%/if%>>[<% $eachone.trunkproto %>] <% $eachone.trunkname %></option>
		<%/foreach%>
			</select>
			</td>
		</tr>
		<tr>
			...
			<td height="80">当外线送入的被叫号码以<input type="text" id="iptext1" name="match_callednum" size="8" value="<% $rule.match_callednum %>">开头且长度为<input type="text" id="iptext1" name="match_calledlen" size="2" value="<% $rule.match_calledlen %>">位.<br>从头开始删掉 <input type="text" id="iptext1" name="replace_calledtrim" size="2" value="<% $rule.replace_calledtrim %>">位且补充信息<input type="text" id="iptext1" name="replace_calledappend" size="8" value="<% $rule.replace_calledappend %>"> 到该号码上.</td>
		</tr>
		<tr>
