...
$().ready(function() {
	...
	<h4>编辑呼叫队列 <% $queue.queuenumber %></h4>
<form id='queue_form' name="queue_form" method="POST" action="?action=do_queue_edit&queuenumber=<% $queue.queuenumber %>" target="takefire">
	<table border="0" cellspacing="0" id='recordoverphone' style='margin:20px'>
		...
		<tr>
			...
			<td height="30"><input type="text" id="iptext1" name="queuenumber" size="8" value="<% $queue.queuenumber %>" class="tipmsg" title="必填,纯数字填写" disabled></td>
		</tr>
		<tr>
			<td height="30" align="left" style="padding-left: 10px;padding-right: 40px">队列名称</td>
			<td height="30"><input type="text" id="iptext1" name="queuename" size="30" value="<% $queue.queuename %>"></td>
		</tr>
		<tr>
			<td height="30" align="left" style="padding-left: 10px;padding-right: 40px">震铃策略</td>
			<td height="30">
			<select size="1" name="strategy">
				<option value="ringall" <% if $queueconf.strategy eq 'ringall' %>selected<%/if%>>坐席全呼</option>
				<option value="roundrobin" <% if $queueconf.strategy eq 'roundrobin' %>selected<%/if%>>轮流呼叫</option>
				<option value="leastrecent" <% if $queueconf.strategy eq 'leastrecent' %>selected<%/if%>>最近接听最少</option>
				<option value="fewestcalls" <% if $queueconf.strategy eq 'fewestcalls' %>selected<%/if%>>完成呼叫最少</option>
				<option value="random" <% if $queueconf.strategy eq 'random' %>selected<%/if%>>随机呼叫</option>
				<option value="rrmemory" <% if $queueconf.strategy eq 'rrmemory' %>selected<%/if%>>记忆轮流呼叫</option>
			</select>
			</td>
		</tr>
		<tr>
	...
	<table border="0" cellspacing="0" id='recordoverphone' style='margin:20px'>
		...
		<tr>
			<td height="40" style="padding-left: 10px;padding-right: 40px"><input type="radio" value="1" <% if $queue.playring eq '1' %>checked<%/if%> name="playring">在队列中播放震铃音</td>
		</tr>
		<tr>
			<td height="40" style="padding-left: 10px;padding-right: 40px"><input type="radio" value="0" <% if $queue.playring eq '0' %>checked<%/if%> name="playring">在队列中播放等待音乐</td>
		</tr>
		<tr>
			<td height="40" style="padding-left: 10px;padding-right: 40px"><input type="checkbox" value="1" <% if $queue.saymember eq '1' %>checked<%/if%> name="saymember">用户接起后拨报坐席号码</td>
		</tr>
		<tr>
			<td height="50" align="left" style="padding-left: 10px;padding-right: 40px">繁忙循环通知周期&nbsp;&nbsp;<input type="text" id="iptext1" name="periodic-announce-frequency" size="4" value="<% $queueconf.periodicannouncefrequency %>" class="tipmsg" title="必填,纯数字填写"> 秒</td>
		</tr>
		<tr>
			<td height="50" colspan='2' style="padding-left: 10px;padding-right: 40px">如果呼叫者已经在队列中等待超过 <input type="text" id="iptext1" name="queuetimeout" size="4" value="<% $queue.queuetimeout %>" class="tipmsg" title="必填,纯数字填写"> 秒，将呼叫者跳转到本地号码 <input type="text" id="iptext1" name="failedon" size="8" value="<% $queue.failedon %>" class="tipmsg" title="必填,纯数字填写"> 处理</td>
		</tr>
	</table>
	<table border="0" cellspacing="0" id='recordoverphone' style='margin:20px'>
		<tr>
		...
		<tr>
			...
			<td height="30"><input type="text" id="iptext1" name="timeout" size="4" value="<% $queueconf.timeout %>" class="tipmsg" title="必填,纯数字填写"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<select size="1" name="announce">
				<option value="" <% if $queue.announce eq '' %>selected<%/if%>>无</option>
				<option value="freeiris/callfromqueue" <% if $queue.announce eq 'freeiris/callfromqueue' %>selected<%/if%>>呼叫来自队列</option>
				<option value="freeiris/silence5" <% if $queue.announce eq 'freeiris/silence5' %>selected<%/if%>>静音五秒</option>
			</select>
			</td>
		</tr>
		<tr>
		...
		<%foreach from=$queue.members_res item=eachone key=keyname %>
							<option value="<% $eachone.accountcode %>"><% $eachone.accountcode %> "<% $eachone.info_name %>" </option>
		<%/foreach%>
						</select>
					</td>
