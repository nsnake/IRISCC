...
<form name="do_ivraction_edit" method="POST" action="?action=do_ivraction_edit&ivrnumber=<% $ivrnumber %>&id=<% $id %>&actmode=<% $actmode %>&return=<% $return %>" target='takefire'>
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<tr>
			<td height="30" align="left" style="padding-left: 10px;padding-right: 40px" >要播放的录音文件变量名称</td>
			<td height="30"><input type="text" id="iptext1" name="playbackvarname" size="20" value="<% $args_array.playbackvarname %>" class="tipmsg" title="必填,数字和英文字母组合."></td>
		</tr>
	</table>
	<table border="0" cellspacing="0" style="margin: 20px">
		<tr>
