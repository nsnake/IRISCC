...
<form name="do_ivraction_edit" method="POST" action="?action=do_ivraction_edit&ivrnumber=<% $ivrnumber %>&id=<% $id %>&actmode=<% $actmode %>&return=<% $return %>" target='takefire'>
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<tr>
			...
			<td height="30"><input type="radio" value="" name="typeof" <%if $args_array.typeof eq ''%>checked<%/if%>>本地号码&nbsp;&nbsp;<input type="radio" value="extension" name="typeof" <%if $args_array.typeof eq 'extension'%>checked<%/if%>>分机&nbsp;&nbsp;<input type="radio" value="queue" name="typeof" <%if $args_array.typeof eq 'queue'%>checked<%/if%>>呼叫队列&nbsp;&nbsp;<input type="radio" value="conference" name="typeof" <%if $args_array.typeof eq 'conference'%>checked<%/if%>>电话会议&nbsp;&nbsp;<input type="radio" value="extenrouter" name="typeof" <%if $args_array.typeof eq 'extenrouter'%>checked<%/if%>>拨出规则</td>
		</tr>
		<tr>
			<td height="30" align="center" width="100%" colspan="2"><hr size="1"></td>
		</tr>
		...
		<tr>
			...
			<td height="30"><input type="checkbox" name="playbackinvalid" value="true" <%if $args_array.playbackinvalid eq 'true'%>checked<%/if%>>播放提示号码不存在</td>
		</tr>
		<tr>
