...
<script language="javascript">
	...
	//----------------动作选择器
	action = new Array();
	var edit_done_selected_gotoivr;
	var edit_done_selected_actpoint;
	var edit_failed_selected_gotoivr;
	var edit_failed_selected_actpoint;
	var edit_other_selected_gotoivr;
	var edit_other_selected_actpoint;
...
<% if $eachfile.ivrnumber eq $args_array.done_gotoivr %>
edit_done_selected_gotoivr = <% $array_number %>;
<%/if%>
<% if $eachfile.id eq $args_array.done_actpoint %>
edit_done_selected_actpoint = <% $keyname %>;
<%/if%>
<% if $eachfile.ivrnumber eq $args_array.failed_gotoivr %>
edit_failed_selected_gotoivr = <% $array_number %>;
<%/if%>
<% if $eachfile.id eq $args_array.failed_actpoint %>
edit_failed_selected_actpoint = <% $keyname %>;
<%/if%>
<% if $eachfile.ivrnumber eq $args_array.other_gotoivr %>
edit_other_selected_gotoivr = <% $array_number %>;
<%/if%>
<% if $eachfile.id eq $args_array.other_actpoint %>
edit_other_selected_actpoint = <% $keyname %>;
<%/if%>
<%/foreach%>
...
<%/foreach%>
	...
	$().ready(function() {
		...
		//启动完成后
		$("#done_select_gotoivr")[0].selectedIndex = edit_done_selected_gotoivr;//选择folderid
		add_filename('done_select_gotoivr','done_select_actpoint');
		$("#done_select_actpoint")[0].selectedIndex = edit_done_selected_actpoint+1;//选择folderid

		$("#failed_select_gotoivr")[0].selectedIndex = edit_failed_selected_gotoivr;//选择folderid
		add_filename('failed_select_gotoivr','failed_select_actpoint');
		$("#failed_select_actpoint")[0].selectedIndex = edit_failed_selected_actpoint+1;//选择folderid

		$("#other_select_gotoivr")[0].selectedIndex = edit_other_selected_gotoivr;//选择folderid
		add_filename('other_select_gotoivr','other_select_actpoint');
		$("#other_select_actpoint")[0].selectedIndex = edit_other_selected_actpoint+1;//选择folderid
	});
...
<form name="do_ivraction_edit" method="POST" action="?action=do_ivraction_edit&ivrnumber=<% $ivrnumber %>&id=<% $id %>&actmode=<% $actmode %>&return=<% $return %>" target='takefire'>
	<table border="0" cellspacing="0" style="margin: 20px">
		...
		<tr>
			...
			<td height="30"><input type="text" id="iptext1" name="urlvar" size="30" value="<% $args_array.urlvar|urldecode %>">&nbsp;&nbsp;超时<input type="text" id="iptext1" name="urltimeout" size="4" value="<% $args_array.urltimeout %>">秒</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td height="30"><input type="text" id="iptext1" name="urlargs" size="40" value="<% $args_array.urlargs|urldecode %>"></td>
		</tr>
		<tr>
