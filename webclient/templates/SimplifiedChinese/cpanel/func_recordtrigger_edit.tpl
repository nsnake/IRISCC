...
$().ready(function() {
	...
	//提交时全选
	$("#do_recordtrigger_edit").submit(function(){
	...
	<h4>编辑自动录音触发器</h4>
<form id='do_recordtrigger_edit' name="do_recordtrigger_edit" method="POST" action="?action=do_recordtrigger_edit&id=<% $resdata.id %>" target="takefire">
	<table border="0" cellspacing="0" style="margin-left: 20px">
		...
		<tr>
			...
			<td height="30"><input type="text" id="iptext1" name="triggername" size="16" value="<% $resdata.triggername %>" class="tipmsg" title="必填."></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td height="25"><input type="checkbox" name="recordout" value="true" <% if $resdata.recordout eq '1' %>checked<%/if%>>拨打电话&nbsp;&nbsp;<input type="checkbox" name="recordin" value="true" <% if $resdata.recordin eq '1' %>checked<%/if%>>接听电话&nbsp;&nbsp;<input type="checkbox" name="recordqueue" value="true" <% if $resdata.recordqueue eq '1' %>checked<%/if%>>作为呼叫队列成员接听</td>
		</tr>
		<tr>
		...
		<tr>
			<td height="25" align="left" style="padding-left: 10px;padding-right: 40px"></td>
			<td height="25"><input type="text" id="iptext1" name="keepfortype0" size="4" value="<% $resdata.keepfortype0 %>" class="tipmsg" title="必填.">条&nbsp;&nbsp;或&nbsp;&nbsp;<input type="text" id="iptext1" name="keepfortype1" size="4" value="<% $resdata.keepfortype1 %>" class="tipmsg" title="必填.">天&nbsp;&nbsp;或&nbsp;&nbsp;<input type="checkbox" name="keepfortype2" value="true" <% $resdata.keepfortype2 %>>永久保存</td>
		</tr>
		<tr>
			<td height="30" align="left" style="padding-left: 10px;padding-right: 40px" valign='top'>选择成员</td>
			<td height="30" >
				<textarea rows="6" id="iptext1" name="members" cols="30" title='请输入录音的对象号码,每行输入一个.' class="tipmsg"><% $members %></textarea>
