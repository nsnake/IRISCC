...
//				$(this).attr('src',gotourl);
	...
	<table border="0" width="90%" align="left" style="margin-top: 20px">
		<tr>
			...
			<table border="0" width="100%" align="left" cellspacing="0" cellpadding="0">
 				<tr>
					...
					<td  align="center" style="border-bottom: #ACA8A1 1px solid;">对方号码</td>
		...
		<%foreach from=$onetouch item=eachone key=keyname %>
 				<tr>
					...
					<td height="30" align="left" style="border-bottom: #ACA8A1 1px solid;" bgcolor="<% if $keyname is even %>#FFFFFF<%else%>#F3F3F3<%/if%>">&nbsp;&nbsp;<% if $eachone.callee ne '' %>拨打 <% $eachone.callee %> 的录音<%else%>来自 <% $eachone.cdr.src %> 的录音<%/if%></td>
					...
					<td height="30" align="center" style="border-bottom: #ACA8A1 1px solid;" bgcolor="<% if $keyname is even %>#FFFFFF<%else%>#F3F3F3<%/if%>">&nbsp;<input type="button" value="删除" name="button" class="deletebtn" id='btn2' gotourl="pbx_uservoice.php?action=do_uservoice_delete&id=<% $eachone.id %>&accountcode=<% $accountcode %>&type=ot"></td>
				</tr>
		<%/foreach%>
			</table>
			</td>
